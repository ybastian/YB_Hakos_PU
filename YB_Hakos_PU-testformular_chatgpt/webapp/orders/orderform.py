...
class ObservationRequestPositionForm(FlaskForm):
    ...
    # optional; may be empty depending on workflow
    exposure_starttime = TimeField('exposure_starttime', format='%H:%M', validators=[Optional()])
    ...
    # Expert fields: must not block saving when expert mode is off / fields are hidden.
    # We accept empty inputs and treat them as None (or default later when persisting).
    exposure_gain   = IntegerField('exposure_gain', validators=[Optional(), NumberRange(min=0)])
    exposure_offset = IntegerField('exposure_offset', validators=[Optional(), NumberRange(min=0)])
    exposure_focus  = IntegerField('exposure_focus', validators=[Optional(), NumberRange(min=0)])
    exposure_dither = IntegerField('exposure_dither', validators=[Optional(), NumberRange(min=0)])
